...
export function validateConfigDocument(
  ...
  for (const [key, value] of Object.entries(raw)) {
    if (key === "mode") {
      ...
      if (!isRecord(value)) throw new ConfigError("diagnostics must be a mapping");
      const settings: NonNullable<ConfigDocument["diagnostics"]> = {};
      for (const [setting, raw] of Object.entries(value)) {
        if (setting === "enabled" && typeof raw === "boolean") settings.enabled = raw;
        else if (setting === "maxRequests" && typeof raw === "number" && Number.isInteger(raw) && raw >= 1 && raw <= 500) settings.maxRequests = raw;
