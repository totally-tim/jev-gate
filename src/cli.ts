...
const read = (path: string) => readFileSync(path === "-" ? 0 : path, "utf8");
export async function runCli(argv: readonly string[]): Promise<number> {
  ...
  try {
    ...
    if (
      !["review", "snapshot", "diff", "calibrate", "resolve"].includes(command)
    )
    ...
    const options: LocalOptions = {
      base: str("base"),
      config: str("config"),
      policySource: policySource as LocalOptions["policySource"],
    ...
    if (
      [str("diff"), str("base"), str("snapshot")].filter((v) => v !== undefined)
        .length > 1
    )
    ...
    const snapshot = str("snapshot")
      ...
      : await collectSnapshot(
          options,
          str("diff") ? read(str("diff")!) : undefined,
        );
    if (command === "snapshot") {
