...
test("missing patches and zero GitHub counts recover from exact head and merge-base", async () => {
  ...
  pr.baseSha = "b".repeat(40); pr.headSha = "c".repeat(40);
  const reads: string[] = [];
  const results = await recoverPatches({
    getMergeBase: async (_o, _r, base, head) => {
      assert.equal(base, pr.baseSha); assert.equal(head, pr.headSha); return "a".repeat(40);
    },
  ...
  assert.equal(results[0]?.additions, 1); assert.equal(results[0]?.deletions, 1);
...
test("added, removed, and renamed files use the correct side and path", async () => {
  ...
  const pr = localContext(files); pr.headSha = "c".repeat(40);
  const reads: string[] = []; let merges = 0;
  ...
  assert.equal(result[0]?.deletions, 0); assert.equal(result[1]?.additions, 0);
...
test("ignored files never fetch content and failed recovery preserves existing partial evidence", async () => {
  ...
  assert.equal(reads, 1); assert.deepEqual(result[0], files[0]);
...
test("recovery stops at file and byte budgets without claiming completion", async () => {
  ...
  assert.equal(reads, 100); assert.match(result[100]?.patchWarning ?? "", /file limit/);
  reads = 0;
  ...
  assert.equal(reads, 8); assert.match(limited[8]?.patchWarning ?? "", /total byte limit/);
});
...
test("small failed reads do not consume full per-file byte allowances", async () => {
  ...
  const result = await recoverPatches({
    ...
    getFileAtRef: async (_o, _r, _p, _s, _max, options) => {
      options?.onBytes?.(4); reads++;
      if (reads < 9) throw new Error("Binary file");
      return "new\n";
    },
  }, localContext(files), files, []);
  assert.equal(reads, 9); assert.match(result[8]?.patch ?? "", /\+new/);
});
...
test("recovery never follows symlinks or submodules through the contents API", async () => {
  let contentReads = 0; let treeReads = 0;
  const client = new GitHubClient("test", async url => {
    ...
    contentReads++; return new Response("ignored target content");
  });
  ...
  assert.equal(treeReads, 1); assert.equal(contentReads, 0);
