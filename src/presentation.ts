import type { Finding, ReviewOutcome, RuleDecision } from "./types.js";
...
export const ruleLabel = (name: string): string => LABELS[name] ?? name;
export const isModelDecision = (decision: RuleDecision): boolean =>
  decision.candidate.id !== "local-secret-scan" && decision.candidate.status !== "local";
...
export function reviewTopics(outcome: ReviewOutcome): ReviewTopic[] {
  const groups = new Map<string, ReviewTopic>();
...
export function estimate(decision: RuleDecision): string {
...
export interface ConcernCell {
  text: string;
  tone: "quiet" | "sensitive" | "concern" | "missing";
  decision?: RuleDecision;
}
export interface ConcernMap {
  rules: string[];
  rows: Array<{ path: string; cells: ConcernCell[] }>;
  totalFiles: number;
}

/** A peak is a locator for a question, never an aggregate probability of a defect. */
export function concernMap(outcome: ReviewOutcome): ConcernMap {
...
export function peakDecision(topic: ReviewTopic, outcome: ReviewOutcome): RuleDecision | undefined {
...
export function thresholdLabel(d: RuleDecision): string {
  return d.kind === "noul" ? `${Math.round(d.threshold * 100)}%` : `${(d.threshold * ((d.levels ?? 2) - 1)).toFixed(1)} / ${(d.levels ?? 2) - 1}`;
}

export const html = (value: string): string => value.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#39;").replace(/[\r\n]/g, " ");

export function estimateGraphic(d: RuleDecision, sensitive = false): string {
