...
test("empty documents resolve to the built-in defaults", () => {
  ...
  assert.equal(config.rules.filter((rule) => rule.gate).length, 3);
  assert.equal(config.comment, true);
  assert.equal(config.borderlineMargin, 0);
});
...
test("rule overrides replace gate and threshold", () => {
  ...
  const rule = config.rules.find(
    (entry) => entry.name === "danger-sensitive-area",
  );
...
test("providers select the default model and validate their settings", () => {
  ...
  assert.equal(typesafe.model, "jev-1.13.0");

  const openrouter = resolveConfig(
    validateConfigDocument({ provider: "openrouter" }),
  );
  assert.equal(openrouter.model, "typesafe/jev-1.13-20260917");
  ...
  assert.deepEqual(withSettings.openrouter, {
    referer: "https://example.test",
    title: "example",
  });

  assert.throws(
    () => validateConfigDocument({ provider: "gemini" }),
    ConfigError,
  );
  assert.throws(
    () => validateConfigDocument({ openrouter: { referer: "" } }),
    ConfigError,
  );
  assert.throws(
    () => validateConfigDocument({ openrouter: { site: "example" } }),
    ConfigError,
  );
});
...
test("edge thresholds produce warnings instead of errors", () => {
  ...
  const doc = validateConfigDocument(
    { rules: { "breaking-change": { threshold: 0 } } },
    warnings,
  );
  ...
  const second: string[] = [];
  validateConfigDocument(
    { rules: { "comment-drift": { threshold: 1 } } },
    second,
  );
  ...
  const clean: string[] = [];
  validateConfigDocument(
    { rules: { "breaking-change": { threshold: 0.6 } } },
    clean,
  );
...
test("borderlineMargin accepts the sane band and rejects the rest", () => {
  const config = resolveConfig(
    validateConfigDocument({ borderlineMargin: 0.25 }),
  );
  assert.equal(config.borderlineMargin, 0.25);
  assert.equal(
    resolveConfig(validateConfigDocument({ borderlineMargin: 0 }))
      .borderlineMargin,
    0,
  );

  assert.throws(
    () => validateConfigDocument({ borderlineMargin: 0.31 }),
    ConfigError,
  );
  assert.throws(
    () => validateConfigDocument({ borderlineMargin: -0.1 }),
    ConfigError,
  );
  assert.throws(
    () => validateConfigDocument({ borderlineMargin: "0.1" }),
...
test("unknown rule names and settings are rejected", () => {
  assert.throws(
    () => validateConfigDocument({ rules: { nonsense: { gate: true } } }),
    ConfigError,
  );
  assert.throws(
    () =>
      validateConfigDocument({ rules: { "breaking-change": { level: 1 } } }),
    ConfigError,
  );
  assert.throws(() => validateConfigDocument({ model: 42 }), ConfigError);
  assert.throws(
    () => validateConfigDocument({ maxStateTokens: 100 }),
    ConfigError,
  );
  assert.throws(
    () =>
      validateConfigDocument({
        rules: { "breaking-change": { threshold: 1.5 } },
      }),
    ConfigError,
  );
