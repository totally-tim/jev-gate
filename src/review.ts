...
export async function runReview(input: ReviewCoreInput): Promise<ReviewOutcome> {
  ...
  const baseRequest: Omit<JevReviewRequest, "questions"> = {
...
function mergeSecondAsk(decisions: RuleDecision[], second: readonly RuleDecision[]): void {
  const byName = new Map(second.map((decision) => [decision.name, decision]));
  for (let index = 0; index < decisions.length; index += 1) {
    const decision = decisions[index] as RuleDecision;
