...
function renderBody(
  ...
  repository?: string,
  compact?: { maxFindings: number; maxCoverage: number },
): string {
  const coverage = outcome.coverage.files,
    active = outcome.findings.filter((f) => f.status === "open");
  const lines = [
...
export function renderSummary(outcome: ReviewOutcome): string {
