import type { ReviewOutcome, RuleDecision, Finding } from "./types.js";
import { concernMap, estimate, estimateGraphic, html, isModelDecision, peakDecision, REVIEW_ASSETS, reviewTopics, ruleLabel, thresholdLabel, type ReviewTopic } from "./presentation.js";
export const COMMENT_MARKER = "<!-- jev-gate:report -->";
const DATA_OPEN = "<!-- jev-gate:data",
  DATA_CLOSE = "-->";
export function parsePreviousOutcome(body: string): ReviewOutcome | null {
  const start = body.indexOf(DATA_OPEN),
    end = body.indexOf(DATA_CLOSE, start + DATA_OPEN.length);
...
export function displayValue(
  d: Pick<RuleDecision, "kind" | "probability" | "level" | "levels">,
): string {
  return d.kind === "noul"
    ? d.probability === null
      ? "unavailable"
...
function location(
  f: Pick<Finding, "path" | "startLine" | "side">,
...
function locationUrl(f: Pick<Finding, "path" | "startLine" | "side">, outcome: ReviewOutcome, repository?: string): string | undefined {
  const sha = f.side === "old" ? outcome.baseSha : outcome.headSha;
  if (!repository || !/^[\w.-]+\/[\w.-]+$/.test(repository) || !/^[a-f0-9]{40}$/i.test(sha)) return undefined;
...
function htmlLocation(f: Pick<Finding, "path" | "startLine" | "side">, outcome: ReviewOutcome, repository?: string, labelOverride?: string): string {
  const label = labelOverride ?? `${f.path}${f.startLine === null ? "" : `:${f.startLine}`}${f.side === "old" ? " (before change)" : ""}`;
  const url = locationUrl(f, outcome, repository);
  return url ? `<a href="${html(url)}">${html(label)}</a>` : html(label);
}

function diagnosticDetails(finding: Finding): string[] {
...
function topicRow(topic: ReviewTopic, outcome: ReviewOutcome, previous: ReviewOutcome | null, repository: string | undefined, limit: number): string {
...
function fileMap(outcome: ReviewOutcome, repository?: string): string {
...
function renderBody(
  ...
  if (notes.length) {
    ...
    for (const note of notes.slice(0, options.observations ? undefined : 20)) lines.push(`- ${escape(note)}`);
  ...
  if (options.observations) {
    ...
    lines.push("<details>", `<summary>Individual model observations (${model.length})</summary>`, "",
...
export function renderComment(outcome: ReviewOutcome, previous: ReviewOutcome | null, repository?: string, reportUrl?: string): string {
  ...
  while (maxFindings > 0) {
    ...
    if (compact.length <= 60_000) return compact;
  }
...
export function renderSummary(outcome: ReviewOutcome, repository?: string): string {
...
export function renderPlainTable(outcome: ReviewOutcome): string {
  ...
  for (const f of outcome.findings)
    lines.push(
      ...
      `  ${f.verification}`,
      ...diagnosticDetails(f).map(text => `  ${text}`),
    );
  ...
  if (outcome.diagnostics) lines.push(`Optional compatibility follow-up: ${outcome.diagnostics.health}; ${outcome.diagnostics.completed}/${outcome.diagnostics.eligible} findings assessed. Gate decisions are unchanged.`);
  return lines.join("\n");
}
