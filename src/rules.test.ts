...
test("noul decisions gate at the threshold", () => {
  ...
  const deletedTests = decisions.find(
    (decision) => decision.name === "danger-deleted-tests",
  );
  assert.equal(deletedTests?.failed, true);
  const commentDrift = decisions.find(
    (decision) => decision.name === "comment-drift",
  );
...
test("score decisions normalize against the rubric length", () => {
  ...
  const decision = evaluate(
    rules.filter((rule) => rule.name === "test-meaningfulness"),
    answers,
    candidate,
  )[0];
...
test("missing or malformed answers become error rows instead of throwing", () => {
  ...
  assert.ok(
    decisions.every(
      (decision) => decision.error !== null && decision.probability === null,
    ),
  );
  assert.ok(
    decisions.every((decision) => !decision.failed && !decision.exceeded),
  );
  const malformed = evaluate(
    rules.filter((rule) => rule.name === "breaking-change"),
    { "breaking-change": { type: "score", score: 1 } },
    candidate,
  );
  ...
  const healthy = evaluate(
    ...
    {
      ...Object.fromEntries(
        rules.map((rule) => [rule.name, { type: "noul", noul: 0.1 }]),
      ),
  ...
  assert.equal(
    healthy.find((decision) => decision.name === "breaking-change")?.failed,
    true,
  );
  assert.equal(
    healthy.find((decision) => decision.name === "comment-drift")?.error,
    null,
  );
});
