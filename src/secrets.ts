...
const ASSIGNMENT =
  /((?:api[_-]?key|secret|password|(?:access|auth|bearer)[_-]?token)["']?\s*[:=]\s*)(?:"([^"\r\n]*)"|'([^'\r\n]*)'|([A-Za-z0-9_./+=-]{16,}))/gi;
const placeholder = (value: string): boolean =>
  /(?:example|placeholder|your[-_]|redacted|dummy|changeme|test[-_]key)/i.test(
    value,
  );
      ...
      line = line.replace(
        ...
        (
          ...
          if (
            ...
            return whole;
          const quote =
            double !== undefined ? '"' : single !== undefined ? "'" : "";
...
export function scanSecrets(files: readonly DiffFile[]): {
  files: DiffFile[];
  findings: Finding[];
} {
        ...
        if (line.startsWith("+") && !line.startsWith("+++")) {
          if (line !== safeLines[i])
            findings.push({
              ...
              side: "new",
              evidence:
                "A local credential pattern matched an added line. The value was withheld from provider requests and review output.",
              verification:
                "Verify the credential locally. Remove it from source and rotate it if it was exposed. Do not paste its value into a review.",
              value: 1,
              kind: "noul",
              source: "local",
              status: "open",
