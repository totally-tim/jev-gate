...
async function fixture(t: TestContext) {
      ...
      Object.entries(body.questions).map(([name, q]) => [
        ...
        (q as { type: string }).type === "choice"
          ? { type: "choice", choice: name === "evidence" ? "R1" : "api", confidence: 1,
              probabilities: Object.fromEntries(Object.keys((q as { criteria: object }).criteria).map(key => [key, key === (name === "evidence" ? "R1" : "api") ? 1 : 0])) }
  ...
  async function run(...args: string[]) {
    try {
      ...
      const r = e as { stdout: string; stderr: string; code: number };
...
test("calibration rejects missing answers instead of reporting a clean evaluation", async (t) => {
  ...
  writeFileSync(
    join(f.dir, "samples", "labels.json"),
    JSON.stringify({
      schema: 1,
      samples: {
