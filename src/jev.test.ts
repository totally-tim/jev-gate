...
test("openrouter retries retryable failures and parses the answers", async () => {
  ...
  assert.equal((result.answers.urgent as { noul: number }).noul, 0.9);
});
...
test("openrouter tolerates an absent usage block", async () => {
  const fetchImpl = async (): Promise<Response> =>
    response(200, { answers: { urgent: { type: "noul", noul: 0.5 } } });
...
test("an unknown provider is refused", async () => {
  await assert.rejects(
    runJevReview({ ...request, provider: "nonsense" as never }),
    /unknown provider/,
  );
});
...
test("local-decide honors the base URL environment and an explicit base", async () => {
  ...
  try {
    ...
    const local = { provider: "typesafe" as const, apiKey: "test", model: "local-decide", state: "x", questions, fetchImpl };
