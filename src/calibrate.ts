...
import type { ResolvedConfig, ReviewOutcome } from "./types.js";
export interface Labels {
  schema: 1;
  samples: Record<
    string,
    { split: "tune" | "holdout"; expected: Record<string, boolean> }
  >;
}
...
export async function calibrate(
  ...
  const names = config.rules.filter((r) => r.enabled).map((r) => r.name),
