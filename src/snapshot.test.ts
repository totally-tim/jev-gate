...
test("local config discovery and base policy use explicit distinct identities", async () => {
  ...
  try {
    ...
    assert.equal(
      working.config.rules.find((r) => r.name === "breaking-change")?.enabled,
      true,
    );
    assert.equal(
      base.config.rules.find((r) => r.name === "breaking-change")?.enabled,
      false,
    );
