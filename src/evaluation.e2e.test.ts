...
for (const missing of [false, true]) test(`evaluation entrypoint measures screening replay (missing answers: ${missing})`, async t => {
  let requests = 0;
  ...
  assert.equal(result.code, missing ? 2 : 0, JSON.stringify(report.records.map((row: any) => ({ replay: row.screeningReplayed, errors: row.errors, diagnostic: row.diagnostic }))));
