...
const config = { mode: "required", diagnostics: { enabled: true } };
function choiceAnswer(label, keys, confidence = 0.9) {
...
function model(options = {}, calls = []) {
    ...
    return async (url, init) => {
        ...
        if (body.questions.evidence) {
            ...
            if (options.badChoice)
                answers.evidence.choice = "hallucinated-region";
        }
        else {
            answers = { mechanism: choiceAnswer(options.mechanism ?? "api", Object.keys(DIAGNOSTIC_POLICY.mechanisms)) };
            if (!options.missingImpact)
                answers.impact = impactAnswer();
        }
...
for (const [options, status, requestCount] of [
    ...
    test(`${status} preserves an open required finding`, async () => {
...
test("diagnostic regions retain every line and use the original side and line numbers", () => {
    for (const candidate of [
        { id: "x", path: "a", status: "removed", side: "old", startLine: 90, endLine: 105, patch: Array.from({ length: 16 }, (_, i) => `-const x${i} = 1;`).join("\n") },
        { id: "x", path: "a", status: "modified", side: "new", startLine: 20, endLine: 35, patch: "@@ -1,16 +20,16 @@\n" + Array.from({ length: 16 }, (_, i) => `+const x${i} = 1;`).join("\n") },
    ]) {
...
test("choice validation preserves distinct confidence but rejects a contradictory selected label", async () => {
    ...
    const run = (choice) => diagnoseCompatibility(candidate, context, async () => ({
