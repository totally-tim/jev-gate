...
export function localFeedbackHash(cwd = process.cwd()) {
    try {
        return hash(readFileSync(join(projectRoot(cwd), ".jev-gate/dispositions.jsonl"), "utf8"));
    }
    catch (error) {
...
export function makeSnapshot(pr, files, config, configSource = "defaults", warnings = [], feedbackHash = hash("")) {
...
export function applyOverrides(config, options) {
...
export function loadLocalConfig(options, baseSha) {
    ...
    if (options.policySource === "base") {
        ...
        if (options.config)
            throw new ConfigError("--config and --policy-source base cannot be combined");
        const listing = execFileSync("git", ["ls-tree", "--name-only", baseSha, "--", ".jev-gate.yml"], { cwd: root, encoding: "utf8" });
        if (listing.trim())
            config = configFromText(execFileSync("git", ["show", `${baseSha}:.jev-gate.yml`], {
                cwd: root,
                encoding: "utf8",
            }));
...
export function localContext(files, options = {}) {
    return {
        owner: "local",
        repo: "local",
        number: 0,
...
export async function collectSnapshot(options, diffText) {
    const local = diffText === undefined ? await localDiff(options.base, options.cwd) : null;
    ...
    return makeSnapshot(pr, parsed.files, policy.config, policy.source, [...(local?.warnings ?? []), ...parsed.warnings], localFeedbackHash(options.cwd));
}
...
export function parseSnapshot(text) {
    ...
    if (typeof value.feedbackHash !== "string" ||
    ...
    if (!Array.isArray(c.rules))
        throw new ConfigError("invalid snapshot rules");
    const config = resolveConfig(validateConfigDocument({
        ...
        rules: Object.fromEntries(c.rules.map((r) => [
            r.name,
    ...
    const rebuilt = makeSnapshot(value.pr, value.files, config, value.configSource, value.warnings, value.feedbackHash);
    if (rebuilt.id !== value.id)
        throw new ConfigError("snapshot content or installed rules changed; collect a fresh snapshot");
    return rebuilt;
}
