...
export function diagnosticRegions(candidate) {
    const regions = [];
    ...
    for (const text of candidate.patch.split("\n")) {
        ...
        if (hunk)
            line = Number(hunk[candidate.side === "old" ? 1 : 2]) || null;
        ...
        lines.push(text);
        if (lines.length >= DIAGNOSTIC_POLICY.regionLines)
            flush();
    }
    flush();
    return regions;
...
function record(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        throw new Error("Missing diagnostic answer");
    return value;
}
function unit(value) {
...
function distribution(value, keys) {
    ...
    if (Math.abs(sum - 1) > 0.005 * keys.length + 0.000001)
        throw new Error("Invalid diagnostic probability total");
...
export async function diagnoseCompatibility(candidate, context, ask) {
    const diagnostic = { status: "unavailable", reason: "Diagnostic did not complete" };
    const regions = diagnosticRegions(candidate);
    if (regions.length > DIAGNOSTIC_POLICY.maxRegions)
        return { status: "skipped", reason: `Candidate exceeds the diagnostic limit of ${DIAGNOSTIC_POLICY.maxRegions} regions; no evidence was clipped` };
    const options = {
    ...
    try {
        ...
        if (selection.choice === "noMatch")
            return { ...diagnostic, status: "no-match", reason: "The follow-up did not select supporting evidence; the original finding remains open" };
        ...
        if (mechanism.choice === "noIssue")
            return { ...diagnostic, status: "no-issue", reason: "The follow-up found compatibility or migration evidence; the original finding remains open" };
