...
function readOpenRouterSettings(raw) {
    if (!isRecord(raw))
        throw new ConfigError("openrouter must be a mapping");
    const settings = {};
    for (const [key, value] of Object.entries(raw)) {
    ...
    for (const [key, value] of Object.entries(raw)) {
        if (key === "provider") {
            ...
            if (typeof value !== "number" ||
                ...
                throw new ConfigError(`maxStateTokens must be an integer between ${MIN_STATE_TOKENS} and ${MAX_STATE_TOKENS}`);
            }
            ...
            if (!isRecord(value))
                throw new ConfigError("rules must be a mapping of rule name to settings");
            const rules = {};
...
export function resolveConfig(doc) {
