...
test("test candidates receive bounded context from their related implementation", () => {
    const testFile = file("@@ -1 +0,0 @@\n-test('legacy', run);", "src/auth.test.ts");
    const implementation = file("@@ -1 +0,0 @@\n-export function legacy() {}", "src/auth.ts");
    const state = buildState(localContext([testFile, implementation]), candidatesFor(testFile, 1000)[0], [testFile, implementation]);
...
test("candidate locations distinguish deleted code from added code", () => {
    const candidates = candidatesFor(file("@@ -15,2 +15,0 @@\n-old\n-code"), 1000);
