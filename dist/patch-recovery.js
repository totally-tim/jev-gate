...
export function patchFromContents(before, after) {
    ...
    try {
        ...
        if (start < 0)
            throw new Error("Git returned no textual hunks");
...
export async function recoverPatches(client, pr, files, ignore) {
    let mergeBase;
    let recovered = 0;
    ...
    const read = async (path, ref) => {
        if (remaining <= 0)
            throw new Error("Patch recovery reached its total byte limit");
        if (Date.now() >= deadline)
            throw new Error("Patch recovery reached its time limit");
        ...
        if (text === null)
            throw new Error("File was unavailable at the pinned revision");
        return text;
    };
    const result = [];
    for (const file of files) {
        ...
        try {
            if (recovered++ >= MAX_FILES)
                throw new Error("Patch recovery reached its file limit");
            ...
            if (!patch)
                throw new Error("No textual change; file metadata was not reviewed");
            result.push({
