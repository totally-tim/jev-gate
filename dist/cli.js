...
function requireApiKey(args, provider) {
    ...
    if (!key)
        throw new ConfigError(`no API key: set ${envKey} or pass --api-key`);
    return key;
}
async function commandReview(args) {
    ...
    if ((outcome.failedGates.length > 0 || outcome.erroredGates.length > 0) && !args.flags.has("no-gate"))
        return 1;
    return 0;
}
...
function summarize(name, values) {
    ...
    if (numbers.length === 0)
        return { name, values, mean: null, min: null, max: null };
    ...
    if (repeat > 1) {
        const spreads = enabledRules.map((rule) => {
            ...
            for (const result of results) {
                const decision = result.decisions.find((entry) => entry.name === rule.name);
                if (!decision || decision.min === null || decision.max === null || decision.values.length < 2)
                    continue;
                observed = true;
        ...
        for (const spread of spreads.filter((entry) => entry.observed)) {
            process.stdout.write(`max run-to-run spread: ${spread.rule.name} ${(spread.worst * 100).toFixed(1)}pp (${spread.worstSample})\n`);
        }
    }
