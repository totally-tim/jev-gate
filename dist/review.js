...
export function finishOutcome(outcome) {
    ...
    const gated = new Set(outcome.decisions.filter((d) => d.gate).map((d) => d.name));
...
export function reviewExitCode(outcome, noGate = false) {
...
export async function runReview(input) {
    ...
    const scanned = scanSecrets(snapshot.files.filter((f) => !isIgnored(f.path, config.ignore)));
    ...
    if (pr.changedFiles > snapshot.files.length)
        outcome.coverage.warnings.push(`Collected ${snapshot.files.length} of ${pr.changedFiles} changed files.`);
    ...
    for (const original of snapshot.files) {
        ...
        if (original.patchWarning)
            outcome.coverage.warnings.push(`${original.path}: ${original.patchWarning}`);
    ...
    // One request carries every applicable rule; the rules test bounds how many questions that packs.
    ...
    // Both providers answer 429 or 529 when saturated; the SDK retries those and honors Retry-After.
            ...
            try {
                ...
                if (borderline.length) {
                    ...
                    for (const d of decisions) {
                        ...
                        if (d.kind === "noul")
                            d.probability = d.value;
                        else
                            d.level = d.value * ((d.levels ?? 2) - 1);
    ...
    for (let index = 0; index < jobs.length; index++) {
        ...
        for (const d of result.decisions.filter((d) => d.exceeded && d.error === null)) {
            ...
            const finding = {
                ...
                rule: d.name,
                title: d.title,
                category: d.name === "danger-sensitive-area"
                    ? "review-request"
                    : "potential-issue",
                path: candidate.path,
            ...
            c.status =
                c.totalChunks > 0 && c.reviewedChunks === c.totalChunks
                    ...
                    : c.reviewedChunks > 0
                        ? "partial"
                        : "unavailable";
    const incomplete = outcome.coverage.warnings.length > 0 ||
        outcome.coverage.files.some((c) => c.status === "partial" || c.status === "unavailable");
