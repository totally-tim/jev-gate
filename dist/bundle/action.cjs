...
var ruleLabel = (name) => LABELS[name] ?? name;
var isModelDecision = (decision) => decision.candidate.id !== "local-secret-scan" && decision.candidate.status !== "local";
function priority(f) {
  return f.source === "local" ? 0 : f.category === "secret" ? 1 : f.category === "potential-issue" ? 2 : 3;
}
function reviewTopics(outcome) {
...
function estimate(decision) {
  if (decision.error || decision.value === null) return "Unavailable";
  return decision.kind === "noul" ? `${Math.round((decision.probability ?? decision.value) * 100)}%` : `${(decision.level ?? decision.value * ((decision.levels ?? 2) - 1)).toFixed(1)} / ${(decision.levels ?? 2) - 1}`;
}
function concernMap(outcome) {
  ...
  return {
    ...
    rows: paths.slice(0, 6).map((path) => ({
      ...
      cells: rules.map((rule) => {
        ...
        if (local.length) return { text: `${local.length} match${local.length === 1 ? "" : "es"}`, tone: "concern" };
        ...
        if (!best) return { text: "Not assessed", tone: "missing" };
...
var REVIEW_ASSETS = "https://raw.githubusercontent.com/totally-tim/jev-gate/main/assets/review/v1";
function peakDecision(topic, outcome) {
  return outcome.decisions.filter((d) => isModelDecision(d) && d.name === topic.rule && !d.error && d.value !== null && Number.isFinite(d.value) && topic.findings.some((f) => f.path === d.candidate.path && f.startLine === d.candidate.startLine && f.side === d.candidate.side)).sort((a, b) => b.value - a.value)[0];
}
function thresholdLabel(d) {
  return d.kind === "noul" ? `${Math.round(d.threshold * 100)}%` : `${(d.threshold * ((d.levels ?? 2) - 1)).toFixed(1)} / ${(d.levels ?? 2) - 1}`;
}
var html = (value) => value.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#39;").replace(/[\r\n]/g, " ");
function estimateGraphic(d, sensitive = false) {
  if (!isModelDecision(d) || d.error || d.value === null) return "Unavailable";
...
// src/render.ts
var COMMENT_MARKER = "<!-- jev-gate:report -->";
var DATA_OPEN = "<!-- jev-gate:data";
var DATA_CLOSE = "-->";
function parsePreviousOutcome(body) {
  const start = body.indexOf(DATA_OPEN), end = body.indexOf(DATA_CLOSE, start + DATA_OPEN.length);
  if (start < 0 || end < 0) return null;
  try {
    const v = JSON.parse(
      body.slice(start + DATA_OPEN.length, end)
    );
    if (v?.schema !== 2 || typeof v.snapshotId !== "string" || typeof v.model !== "string" || typeof v.policyHash !== "string" || !Array.isArray(v.findings) || !Array.isArray(v.decisions))
      return null;
    if (v.findings.some(
      (f) => !f || typeof f.id !== "string" || typeof f.rule !== "string" || typeof f.path !== "string"
    ))
      return null;
    if (v.decisions.some(
      (d) => !d || typeof d.name !== "string" || !(d.value === null || typeof d.value === "number")
    ))
      return null;
    return v;
  } catch {
...
var escape = (s) => s.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replace(/[\\`*_[\]{}()|!]/g, (c) => `&#${c.charCodeAt(0)};`).replaceAll("\n", " ").replaceAll("\r", "");
...
function locationUrl(f, outcome, repository) {
  ...
  if (!repository || !/^[\w.-]+\/[\w.-]+$/.test(repository) || !/^[a-f0-9]{40}$/i.test(sha)) return void 0;
...
function workflowLink(url) {
  return url && /^https:\/\/[a-z0-9.-]+\/[\w.-]+\/[\w.-]+\/actions\/runs\/\d+$/i.test(url) ? `[Full report](${url})` : "Full report in the workflow summary";
}
function topicTitle(topic) {
  if (topic.source === "local") return "Verify credential matches";
  const titles = {
...
function fileMap(outcome, repository) {
  ...
  return [
    "<details>",
    `<summary>Explore by file (${map.rows.length} of ${map.totalFiles} files with findings)</summary>`,
    "",
    ...
    "",
    `<table><thead><tr><th align="left">File</th>${headers}</tr></thead><tbody>`,
    rows,
    "</tbody></table>",
    "",
    "</details>",
...
function renderBody(outcome, previous, repository, reportUrl, options) {
  ...
  const title = outcome.health === "unavailable" ? "Review unavailable" : outcome.health !== "complete" ? "Review incomplete" : topics.length ? `Review ${topics.length} topic${topics.length === 1 ? "" : "s"}` : "No findings to review";
  ...
  const policy = outcome.mode === "required" ? `Required policy: ${outcome.health !== "complete" ? "unavailable" : outcome.passed ? "passed" : "blocked"}.${outcome.failedGates.length ? ` Gates needing attention: ${outcome.failedGates.map(ruleLabel).map(escape).join(", ")}.` : ""}` : "Advisory \xB7 does not block merging";
  const lines = [
    `### <img src="${REVIEW_ASSETS}/mark.svg" width="24" height="24" alt=""> JEV review`,
    "",
    `> [!${alert}]`,
    `> **${title}**`,
    ">",
  ...
  if (outcome.health !== "complete") lines.push("Coverage is incomplete. Findings cover only the available input; an empty list does not establish that the change was reviewed.", "");
  if (topics.length) {
    lines.push("<table>", '<thead><tr><th align="left">Review focus</th><th align="center">Signal</th><th align="left">What to check \xB7 expand for evidence</th></tr></thead>', "<tbody>");
  ...
  if (settled.length) {
    ...
    for (const f of settled.slice(0, options.maxFindings)) lines.push(`- ${location(f, outcome, repository)}: ${f.status}. ${escape(f.disposition?.reason ?? "")} Finding \`${escape(f.id)}\`.`);
    if (settled.length > options.maxFindings) lines.push(`${settled.length - options.maxFindings} additional dispositions are in the full workflow summary.`);
    lines.push("", "</details>", "");
  }
  ...
  if (notes.length) {
    ...
    for (const note of notes.slice(0, options.observations ? void 0 : 20)) lines.push(`- ${escape(note)}`);
    if (!options.observations && notes.length > 20) lines.push(`- ${notes.length - 20} more notes are in the full workflow summary.`);
    lines.push("", "</details>", "");
  }
  if (options.observations) {
    ...
    lines.push(
      "<details>",
      `<summary>Individual model observations (${model.length})</summary>`,
      "",
      "Estimates describe the rule's question. They are not calibrated measures of defect likelihood.",
      "",
      "| Question | Location | Observation | Review threshold | Result |",
      "| --- | --- | --- | --- | --- |"
    );
  ...
  lines.push(
    ...
    "",
    `- Reviewed head: ${escape(outcome.headSha || "uncommitted input")}.`,
    `- Snapshot: ${escape(outcome.snapshotId)}.`,
    `- Policy: ${escape(outcome.configSource)} (${outcome.mode}).`,
    `- Provider: ${escape(outcome.provider)}. Model: ${escape(outcome.model)}.`,
    `- Rules: ${escape(outcome.rulesHash)}.`,
    `- ${outcome.inputTokens} input tokens; estimated $${outcome.costUSD.toFixed(6)}; ${Math.round(outcome.latencyMs)} ms across model calls.`,
    "",
    "</details>",
    ""
  );
  if (reportUrl) lines.push(workflowLink(reportUrl), "");
  return lines.join("\n");
}
...
function renderSummary(outcome, repository) {
  return renderBody(outcome, null, repository, void 0, { maxFindings: outcome.findings.length, observations: true, map: true }) + "\n";
}
  ...
  async listChangedFiles(owner, repo, number) {
    ...
    for (let page = 1; page <= 30; page += 1) {
      ...
      for (const file of batch) {
        ...
        const incomplete = lines && (lines.filter((line) => line.startsWith("+")).length !== file.additions || lines.filter((line) => line.startsWith("-")).length !== file.deletions);
        files.push({
          path: file.filename,
          status: file.status,
          ...
          patchWarning: incomplete ? "GitHub patch line counts do not match the declared change; the patch may be incomplete" : void 0
        });
      }
  ...
  async findPreviousRun(owner, repo, number) {
    for (let page = 1; page <= 30; page += 1) {
      ...
      for (const comment of comments) {
        if (comment.user?.login === this.commentAuthor && comment.user.type === "Bot" && comment.body?.startsWith(COMMENT_MARKER)) {
          return {
            id: comment.id,
...
async function runAction() {
  ...
  if (!sameRevision(pr, afterCollection) || event.pull_request?.head?.sha && event.pull_request.head.sha !== pr.headSha || event.pull_request?.base?.sha && event.pull_request.base.sha !== pr.baseSha)
  ...
  } else if (config.comment && publishComment && collectionWarnings.every((warning) => warning === configurationError)) {
    try {
  ...
  setOutput(
    "passed",
    outcome.health !== "complete" ? "unavailable" : outcome.passed ? "true" : "false"
  );
...
var isMain = isMainModule(void 0);
if (isMain) {
  runAction().then((code) => {
